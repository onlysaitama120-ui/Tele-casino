...
async def spin_slots(session: AsyncSession, user_id: int, bet: int):
    ...
    # Spin
    symbols = [random.choice(config.SLOTS["symbols"]) for _ in range(3)]
    result_str = "".join(symbols)

    # Calculate payout
    multiplier = config.SLOTS["payouts"].get(result_str, 0)

    # Check partial matches
    if multiplier == 0:
        if symbols[0] == symbols[1]:
            partial = symbols[0] * 2
            multiplier = config.SLOTS["payouts"].get(partial, 0)
    ...
    if won > 0:
        ...
        win_tx = Transaction(
            ...
            amount=won,
            balance_after=wallet.coins,
            description=f"Slots win ({result_str})"
        )
        session.add(win_tx)
    else:
        tx = Transaction(
            ...
            amount=-bet,
            balance_after=wallet.coins,
            description=f"Slots spin ({result_str})"
        )
        session.add(tx)
